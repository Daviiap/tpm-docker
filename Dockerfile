...
RUN git clone https://github.com/tpm2-software/tpm2-tss.git ${src}/tpm2-tss
...
# Installing tpm-tss and tpm2-tools
RUN apt install -y tpm2-tools
...
# Defines the TMP2-tools TCTI
ENV TPM2TOOLS_TCTI swtpm

ENTRYPOINT ["swtpm", "socket", "--tpmstate", "dir=/myswtpm", "--tpm2", "--server", "type=tcp,port=2321", "--ctrl", "type=tcp,port=2322", "--flags", "not-need-init,startup-clear", "--log", "level=20"]
